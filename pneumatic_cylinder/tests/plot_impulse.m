...
function fig = plot_impulse()
    ...
    % plotting
    fig = figure("Name","pneumatic_cylinder impulse response (from mu_c to all states)"); hold on;
    p1 = plot(simOut_simscape.logsout.getElement("x").Values*diag([1e3,1e3,1e-3,1e-3]),"-r","LineWidth",1);
    p2 = plot(simOut_ode.logsout.getElement("x").Values*diag([1e3,1e3,1e-3,1e-3]),"--b","LineWidth",1);
    p3 = plot(t_sysc,x_sysc(:,:,1)*1e-6*diag([1e3,1e3,1e-3,1e-3]),"-.k","LineWidth",1);
