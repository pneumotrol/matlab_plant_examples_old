% plot bode diagram from torque tau to ball position q, beam angle theta
function figs = plot_bode()
    ...
    % from torque tau to ball position q
    figs(1) = figure("Name","ball_beam bode plot (from tau to q)");
    plot_bode_sub(w_vec,H_simscape(:,1),H_ode(:,1),H_sysc(:,1));

    % from torque tau to beam angle theta
    figs(2) = figure("Name","ball_beam bode plot (from tau to theta)");
